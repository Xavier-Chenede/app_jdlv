                       ...
                       sidebarLayout(
                         ...
                         mainPanel(
                           fluidRow(
                             ...
                             numericInput("stop", label="Stop evolution at generation:"    , min=1,  value= NULL, width= "300px"),
    ...
    server <- function(input, output,session) {
        ...
        observe({
          # run next gen and update plotOutput when timer fires
          if (rt()) {
            data_to_inject <<- newgen(data_to_inject)
            g <<- g+1 
            output$lab_gen <- renderText(glue("Generation {g}"))
           
             # update plotOutput whenever timer fires
            output$gen_grid <- renderPlot({
              plot(
                data_to_inject,
                main = "",
        ...
        # start timer when Go button is clicked
        observeEvent(input$Go, {
